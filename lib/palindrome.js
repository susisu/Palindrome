...
Palindrome.prototype = Object.create(Object.prototype, {
    ...
    "extract": {
        "writable"    : true,
        "configurable": true,
        "value": function extract(input, minLength, minEntropy) {
            // mecab-async のバグのため
            MeCab.command = this.mecab.command;
            ...
            for (i = 0; i < words.length; i++) {
                ...
                for (var j = i; j < words.length; j++) {
                    ...
                    var result = isPalindrome(ws, minLength, minEntropy);
                    if (result[0]) {
                        longest      = result[1];
                        longestCount = result[2];
                        lastIndex    = j;
                    }
                    // 句点で終わっていたら区切る
                    if (words[j][2] === "句点") {
                ...
                if (longest.length > 0) {
                    palindromes.push([longest, longestCount]);
                    // 回文の終了地点まで飛ばす
                    i = lastIndex;
                }
            }
...
function isPalindrome(words, minLength, minEntropy) {
    ...
    if (reading === reading.split("").reverse().join("")) {
        var sentence = joinWords(words);
        // 文が平仮名と記号のみからなる場合
        if (/^[ぁ-ゖー〜、。！？\!\?・…]+$/.test(sentence)) {
            var s = entropy(sentence);
            console.log(s);
                // エントロピーが閾値以上なら回文とする
            if (s >= minEntropy) {
                return [true, sentence, reading.length];
            }
        }
        else {
            return [true, sentence, reading.length];
